...
class SortablePlaylist extends Component {
  ...
  onChangeOrder = (i, order) => {
    this.props.onPlaylistReorder(order);
  };
  ...
  render() {
    ...
    let list = data.length ? (
      ...
      <Text style={appStyles.centerMessage}>
        Você ainda não<Text style={appStyles.bold}> buscou </Text>nenhuma rádio.
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onPlaylistReorder: order => dispatch(playlistReorder(order))
  };
};
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withNavigation(SortablePlaylist));
